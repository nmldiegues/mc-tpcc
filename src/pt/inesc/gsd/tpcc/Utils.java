...
public class Utils {
   ...
   public static URLClassLoader buildProductSpecificClassLoader(String productName, ClassLoader parent) throws Exception {
      System.err.println("Using smart class loading");
      File libFolder = new File(PLUGINS_DIR + File.separator + productName + File.separator + "lib");
      List<URL> jars = new ArrayList<URL>();
      if (!libFolder.isDirectory()) {
         System.err.println("Could not find lib directory: " + libFolder.getAbsolutePath());
      } else {
         String[] jarsSrt = libFolder.list(new FilenameFilter() {
            public boolean accept(File dir, String name) {
               ...
               if (fileName.endsWith("JAR") || fileName.toUpperCase().endsWith("ZIP")) {
                     System.err.println("Accepting file: " + fileName);
                  return true;
               } else {
                     System.err.println("Rejecting file: " + fileName);
                  return false;
               }
            }
         });
   ...
   public static String getCacheWrapperFqnClass(String productName) {
      ...
      if (!file.exists()) {
         System.err.println("Could not find a plugin descriptor : " + file);
         return null;
      }
   ...
   public static File createOrReplaceFile(File parentDir, String actualFileName) throws IOException {
      ...
      if (outputFile.createNewFile()) {
         System.err.println("Successfully created report file:" + outputFile.getAbsolutePath());
      } else {
         System.err.println("Failed to create the report file!");
      }
      return outputFile;
   }
   ...
   public static void backupFile(File outputFile) {
      if (outputFile.exists()) {
         ...
         if (!old.exists()) {
            if (old.mkdirs()) {
               System.err.println("Issues whilst creating dir: " + old);
            }
         }
         String fileName = outputFile.getName() + ".old." + System.currentTimeMillis() + extension;
         File newFile = new File(old, fileName);
         System.err.println("A file named: '" + outputFile.getAbsolutePath() + "' already exists. Moving it to '" + newFile + "'");
